...
//Method that checks empty fields and returns the user to contact with the valid info autofilled
  if (empty($fullName) || empty($serviceTitle) || empty($rating) || empty($description) || empty($otc)) {
    header("Location: ../about.php?error=emptyfields&fn=".$fullName."&st=".$serviceTitle."&description=".$description);
    exit();
  }
  ...
  elseif (!preg_match("/^[a-zA-Z.\s]*$/",$fullName)) {
    header("Location: ../about.php?error=invalidfullName&st=".$serviceTitle."&description=".$description);
    exit();
  }
  ...
  elseif (!preg_match("/^[a-zA-Z\s]*$/",$serviceTitle)) {
    header("Location: ../about.php?error=invalidserviceTitle&fn=".$fullName."&description=".$description);
    exit();
  }
  ...
  elseif (!preg_match("/^[0-9.\s]*$/",$rating)) {
    header("Location: ../about.php?error=invalidRating&fn=".$fullName."&st=".$serviceTitle."&description=".$description);
    exit();
  }

      //Method that checks if the message entered is valid and returns the user to contact with the valid info autofilled
  elseif (!preg_match("/^[a-zA-Z0-9.?,!\s]*$/",$description)) {
    header("Location: ../about.php?error=invalidDescription&fn=".$fullName."&st=".$serviceTitle);
    exit();
  }
  ...
  elseif (!preg_match("/^[0-9\s]*$/",$otc)) {
    header("Location: ../about.php?error=invalidOtc&fn=".$fullName."&st=".$serviceTitle."&description=".$description);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
       header("Location: ../about.php?error=connectionError&fn=".$fullName."&st=".$serviceTitle."&description=".$description);
      exit();
    }
     else {
      ...
      if ($resultCheck > 0) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          mysqli_stmt_execute($stmt);
          header("Location: ../about.php?review-submit=success");
          exit();
        }  
      }
      else {
        header("Location: ../about.php?error=wrongOtc&fn=".$fullName."&st=".$serviceTitle."&description=".$description);
        exit();
      }
    }
  }
